"""
Module for automate_integral_df.py
This module is for pressure greater than or equal to 2000psia
or ppr greater than or equal to 2.0
"""
import os
import pandas as pd
import numpy as np
...
csv_file_path = os.path.join(script_dir, 'sukkarcornelintegral.csv')
...
def locate_and_get_cell_values(reduced_pressure):
    ...
    value_above = df['pseudoreduced_pressures'][ppr_cell_index - 1]
    value_below = df['pseudoreduced_pressures'][ppr_cell_index + 1]
...
def sciv_for_cell_above_ppr(reduced_temp, reduced_pressure):
    ...
    try:
        ...
        if not 1.5 <= reduced_temp <= 1.7:
            ...
            return df.iloc[target_ppr_index, target_tpr_index]
...
def pseudo_reduced_bhp(reduced_temp, reduced_pressure, scrhs):
    ...
    try:
        ...
        abs_diffs = np.abs(df[target_column] - pivot_sciv(reduced_temp, reduced_pressure, scrhs))
        closest_index = abs_diffs.idxmin()
        closest_value = df[target_column][closest_index]
